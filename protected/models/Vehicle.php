...
class Vehicle extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('name, capacity, line_id', 'required'),
			array('capacity, line_id', 'numerical', 'integerOnly'=>true),
			array('name, type', 'length', 'max'=>30),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, name, capacity, type, line_id', 'safe', 'on'=>'search'),
		);
	}
